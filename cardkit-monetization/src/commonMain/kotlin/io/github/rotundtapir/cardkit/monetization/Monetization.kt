 ...
 * a proprietary ad or billing symbol — or a platform type like an Activity — directly.
 * Implementations capture whatever host handle they need (an Activity, a Context, the browser
 * window) at construction.
 *
 * Implementations:
 *  - [io.github.rotundtapir.cardkit.monetization.foss.FossMonetization] (Android) — no ads,
 *    "remove ads" opens a donation page. Lives in this module (no proprietary dependencies).
 *    Used by F-Droid/GitHub builds.
 *  - `BrowserMonetization` (wasmJs) — the same no-ads behaviour for web builds.
 *  - `PlayMonetization` in `cardkit-monetization-play` — real Google Mobile Ads + Play Billing.
 *    Only an app's `play` build flavor depends on that module.
 */
interface Monetization {
    /**
     * Whether ads should be hidden — `true` when the user has purchased "remove ads" (Play) or always
     * `true` in FOSS/browser builds. UI observes this to decide whether to reserve space for a banner.
     */
     ...
     */
    fun maybeShowInterstitial(onDismissed: () -> Unit = {})

    /**
     * Either launch the "remove ads" purchase flow (Play) or open the donation page (FOSS/browser),
     * depending on the implementation.
     */
    fun launchRemoveAdsOrDonate()
