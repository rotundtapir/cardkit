...
class WireShapeTest {
    ...
    private fun <T : ClientMessage> roundTripClient(msg: T) {
        val encoded = json.encodeToString(ClientMessageSerializer, msg)
        assertEquals(msg, json.decodeFromString(ClientMessageSerializer, encoded), "client round-trip: $encoded")
    }

    private fun <T : ServerMessage> roundTripServer(msg: T) {
        val encoded = json.encodeToString(ServerMessageSerializer, msg)
        assertEquals(msg, json.decodeFromString(ServerMessageSerializer, encoded), "server round-trip: $encoded")
    }
    ...
    fun `shared client messages keep the shapes 500 shipped`() {
        assertEquals(
            ...
            json.encodeToString(ClientMessageSerializer, Hello(1, "0.3.0", Platform.ANDROID)),
        )
        assertEquals(
            """{"type":"hello","protocolVersion":1,"appVersion":"0.3.0","platform":"android",""" +
                """"buildFlavor":"foss","commit":"6f7e099"}""",
            json.encodeToString(ClientMessageSerializer, 
                Hello(1, "0.3.0", Platform.ANDROID, buildFlavor = Distribution.FOSS, commit = "6f7e099"),
            ),
        )
        assertEquals(
            """{"type":"lobby.join","code":"AB12","displayName":"Alice"}""",
            json.encodeToString(ClientMessageSerializer, JoinLobby("AB12", "Alice")),
        )
        assertEquals(
            """{"type":"lobby.pickSeat","seat":2}""",
            json.encodeToString(ClientMessageSerializer, PickSeat(Seat(2))),
        )
        assertEquals(
            """{"type":"lobby.start"}""",
            json.encodeToString(ClientMessageSerializer, StartGame),
        )
        assertEquals(
            """{"type":"emote","emote":"wellPlayed"}""",
            json.encodeToString(ClientMessageSerializer, SendEmote(Emote.WELL_PLAYED)),
        )
    }
    ...
    fun `shared server messages keep the shapes 500 shipped`() {
        assertEquals(
            ...
            json.encodeToString(ServerMessageSerializer, ErrorMessage(ErrorCode.BAD_NAME, "try again")),
        )
        assertEquals(
            """{"type":"game.seatStatus","seat":1,"status":"botSubstitute"}""",
            json.encodeToString(ServerMessageSerializer, SeatStatus(Seat(1), OccupancyStatus.BOT_SUBSTITUTE)),
        )
        assertEquals(
            """{"type":"game.over","winnerTeam":0,"scores":{"0":520,"1":260}}""",
            json.encodeToString(ServerMessageSerializer, GameOver(0, mapOf(0 to 520, 1 to 260))),
        )
        assertEquals(
            """{"type":"welcome","sessionToken":"tok","serverVersion":"0.3.0"}""",
            json.encodeToString(ServerMessageSerializer, Welcome("tok", "0.3.0")),
        )
        assertEquals(
            """{"type":"lobby.disbanded","reason":"idleTimeout"}""",
            json.encodeToString(ServerMessageSerializer, LobbyDisbanded(DisbandReason.IDLE_TIMEOUT)),
        )
    }
    ...
    fun `a generic payload is inlined with no extra nesting`() {
        assertEquals(
            ...
            json.encodeToString(ClientMessageSerializer, SubmitAction(5, ToyAction.Move(3))),
        )
        assertEquals(
            """{"type":"game.view","stateVersion":7,"view":{"seat":0,"hand":["AS","KH"],""" +
                """"handSizes":{"0":10,"1":10}},"turnRemainingMillis":30000}""",
            json.encodeToString(ServerMessageSerializer, ViewUpdate(7, view(), turnRemainingMillis = 30_000)),
        )
        assertEquals(
            ...
            json.encodeToString(ServerMessageSerializer, 
                LobbyState(
                    joinCode = "AB12",
    ...
    fun `a game's own create-lobby request rides the shared discriminator`() {
        assertEquals(
            ...
            json.encodeToString(ClientMessageSerializer, ToyCreateLobby("Bob")),
        )
        // Routing is on the shared interface, so the server never names the game's concrete type.
        val decoded = json.decodeFromString(ClientMessageSerializer, 
    ...
    fun `unknown fields are ignored for forward compatibility`() {
        ...
        assertEquals(JoinLobby("AB12", "Alice"), json.decodeFromString(ClientMessageSerializer, encoded))
    }
    ...
    fun `unknown enum values coerce to UNKNOWN so new emotes never break old clients`() {
        ...
        assertEquals(EmoteReceived(Seat(0), Emote.UNKNOWN), json.decodeFromString(ServerMessageSerializer, encoded))
    }
    ...
    fun `an unregistered message type fails to decode rather than decoding as null`() {
        ...
        assertFailsWith<SerializationException> {
            json.decodeFromString(ClientMessageSerializer, """{"type":"lobby.telepathy","wish":"win"}""")
        }
    }

    @Test
    fun `Seat serializes as a bare int and works as a JSON map key`() {
        val encoded = json.encodeToString(ServerMessageSerializer, ViewUpdate(1, view()))
